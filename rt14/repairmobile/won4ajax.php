...
echo "<tr><td>";
echo "<button  type=button onClick=\"parent.location='attachment.php?func=get&attach_id=$attach_id'\" ><i class=\"fa fa-download fa-lg\"></i> $attach_title</button>";
...
echo "<tr><td>";
echo "<button type=button onClick=\"parent.location='attachment.php?func=get&attach_id=$attach_id2'\"><i class=\"fa fa-download\"></i> $attach_title2</button>";
...
echo "<tr><th>".pcrtlang("Assigned To").": </th><td>";
if ($pcwa == "") {
echo pcrtlang("Not Assigned");
} else {
echo "$pcwa";
}
echo "</td></tr>";
...
if ($assigneduser != "") {
echo "<tr><th>".pcrtlang("Assigned To").": </th><td><font class=textblue12b>$assigneduser</font><br>";
...
echo "<tr><th><i style=\"color:#ff0000\" class=\"fa fa-eye fa-lg fa-fw\"></i> ".pcrtlang("Customer Status Check").": </th><td>$totalcustchecks ".pcrtlang("$thetimes")." &bull; ".pcrtlang("Last Check").": $lasttimef</td></tr>";
...
echo "<form action=pc.php?func=stat_change method=post data-ajax=\"false\"><input type=hidden value=$pcwo name=woid><select name=statnum onchange='this.form.submit()'><option value=1>$boxtitles[1]</option><option value=2>$boxtitles[2]</option><option value=8>$boxtitles[8]</option><option value=9>$boxtitles[9]</option><option value=3>$boxtitles[3]</option>";
...
echo "<form action=pc.php?func=changewa method=post data-ajax=\"false\"><select name=workarea onchange='this.form.submit()'>";
echo "<option>".pcrtlang("Set Workarea")."</option>";
if ($pcwa == "") {
echo "<option selected value=\"\">".pcrtlang("Not Assigned")."</option>";
} else {
echo "<option value=\"\">".pcrtlang("Not Assigned")."</option>";
}
...
echo "</select><input type=hidden name=woid value=\"$pcwo\"></form>";
}

echo "<form action=pc.php?func=checkout method=post data-ajax=\"false\"><input type=hidden name=woid value=$pcwo>";
if($rs_chargewhatcount == "0") {

echo "<button type=button onclick=\"this.disabled=true; this.form.submit();\"><i class=\"fa fa-sign-out fa-lg\"></i> ".pcrtlang("CHECKOUT")."</button>";
} else {
echo "<button type=button onclick=\"javascript: if(confirm('".pcrtlang("This Work Order has charges or invoices specified. Are you sure you want to checkout WITHOUT FINALIZING A SALE?")."')) this.form.submit();\"><i class=\"fa fa-sign-out fa-lg\"></i> ".pcrtlang("CHECKOUT")."</button>";
}
echo "</form>";
...
echo "<center>";
if($icon != "") {
echo pcrtlang("Current Priority").": <br><img src=../repair/images/$icon class=menuicon>&nbsp;&nbsp;&nbsp;";
}
...
echo "<center>".pcrtlang("Current Call Status").":<br> <img src=../repair/images/$called"."called.png style=\"width:20px;height:20px;\"> $calledtext</center>";
...
echo "<button type=button onClick=\"parent.location='pc.php?func=addassetphoto&pcid=$pcid&woid=$pcwo'\"><i class=\"fa fa-camera fa-lg\"></i> ".pcrtlang("Upload Asset Photos")."</button>";
...
$finddeposits = "SELECT * FROM deposits WHERE woid = '$pcwo'";
...
echo pcrtlang("Status").": $dstatus</td></tr></table>";
echo "<br>";
}


if ($pcgroupid == 0) {
echo "<button type=button onClick=\"parent.location='../storemobile/deposits.php?woid=$pcwo&cfirstname=$pcname2&ccompany=$pccompany2&caddress=$pcaddress12&caddress2=$pcaddress22&ccity=$pccity2&cstate=$pcstate2&czip=$pczip2&cphone=$pcphone2&cemail=$pcemail2'\"><i class=\"fa fa-plus\"></i> ".pcrtlang("Add a Deposit")."</button>";
} else {
echo "<button type=button onClick=\"parent.location='../storemobile/deposits.php?woid=$pcwo&cfirstname=$ue_pcgroupname&ccompany=$ue_grpcompany&caddress=$ue_grpaddress&caddress2=$ue_grpaddress2&ccity=$ue_grpcity&cstate=$ue_grpstate&czip=$ue_grpzip&cphone=$ue_grpphone&cemail=$ue_grpemail'\"><i class=\"fa fa-plus\"></i> ".pcrtlang("Add a Deposit")."</button>";
}
...
if($pcgroupid == 0) {
echo "<button type=button onClick=\"parent.location='repinvoice.php?func=createinvoice&woid=$pcwo&pcname=$pcname2&pccompany=$pccompany2&pcphone=$pcphone2&pcemail=$pcemail2&pcaddress=$pcaddress12&pcaddress2=$pcaddress22&pccity=$pccity2&pcstate=$pcstate2&pczip=$pczip2&preinvoice=0'\" > ".pcrtlang("Create Invoice (Final) or Quote")."</button>";
} else {
echo "<button type=button onClick=\"parent.location='repinvoice.php?func=createinvoice&woid=$pcwo&pcname=$ue_pcgroupname&pccompany=$ue_grpcompany&pcphone=$ue_grpphone&pcemail=$ue_grpemail&pcaddress=$ue_grpaddress&pcaddress2=$ue_grpaddress2&pccity=$ue_grpcity&pcstate=$ue_grpstate&pczip=$ue_grpzip&preinvoice=0'\" > ".pcrtlang("Create Invoice (Final) or Quote")."</a>";
}
}
...
if ($checkcartcontents > 0) {
if($pcgroupid == 0) {
echo "<button type=button onClick=\"parent.location='repinvoice.php?func=createinvoice&woid=$pcwo&pcname=$pcname2&pccompany=$pccompany2&pcphone=$pcphone2&pcemail=$pcemail2&pcaddress=$pcaddress12&pcaddress2=$pcaddress22&pccity=$pccity2&pcstate=$pcstate2&pczip=$pczip2&preinvoice=1'\" >".pcrtlang("Create Invoice (Prepaid)")."</button>";
} else {
echo "<button type=button onClick=\"parent.location='repinvoice.php?func=createinvoice&woid=$pcwo&pcname=$ue_pcgroupname&pccompany=$ue_grpcompany&pcphone=$ue_grpphone&pcemail=$ue_grpemail&pcaddress=$ue_grpaddress&pcaddress2=$ue_grpaddress2&pccity=$ue_grpcity&pcstate=$ue_grpstate&pczip=$ue_grpzip&preinvoice=1'\">".pcrtlang("Create Invoice (Prepaid)")."</button>";
}
...
echo "<tr><td>$invname</td></tr><tr><td>$invdate2</td></tr><tr><td>$money$invtotal</td></tr>";
if ($iorq != "quote") {
...
echo "<tr><td>$invname_2</td></tr><tr><td>$invdate2_2</td></tr><tr><td>$money$invtotal_2</td></tr>";
if ("$iorq_2" != "quote") {
echo "<tr><td>".pcrtlang("Status: Open")."</td></tr>";
}
if ($invbyuser_2 != "") {
...
echo "<br>";
echo "<button  type=button onClick=\"parent.location='pc.php?func=addmiles&woid=$pcwo&addystring=$addystring'\"><i class=\"fa fa-car fa-lg\"></i> ".pcrtlang("add mileage")."</button>";
...
echo "<input name=timername><button type=submit ><i class=\"fa fa-clock-o fa-lg\"></i> ".pcrtlang("Start New Timer")."</button></form>";
...
echo "<th style=\"text-align:right\"><a href=\"javascript:void(0);\" id=addcnote class=\"ui-btn ui-corner-all ui-shadow ui-btn-inline\"><i class=\"fa fa-plus\"></i></a></th></tr>";
...
echo "<a href=\"javascript:void(0);\" id=addtnote class=\"ui-btn ui-corner-all ui-shadow ui-btn-inline\"><i class=\"fa fa-plus\"></i></a></th></tr>";
