...
start_box();
echo "<h4>".pcrtlang("Swipe Credit Card NOW!")." (Payeezy)</h4>";


echo "<form name=c action=$securedomain/Payeezy.php?func=add2 method=post><input autofocus class=textbox type=password name=swipedata autocomplete=off>";
...
start_blue_box(pcrtlang("Add Credit Card Payment")." (Payeezy)");
if($isdeposit != 1) {
echo "<h4>".pcrtlang("Balance").": $money".mf("$currenttotal")."</h4>";
}
echo "<table><tr><td><form action=$securedomain/Payeezy.php?func=add3 method=post>";
...
require("FirstDataApi/FirstData.php");
...
$firstData = new FirstData($payeezy_gatewayid, $payeezy_password, true);
...
$isapproved = "no";
$firstdataerror = "Error....";
} else {
$isapproved = "yes";
$cc_transid = $firstData->getAuthNumber();
...
$rs_insert_gcc = "INSERT INTO currentpayments (pfirstname,pcompany,byuser,amount,paymentplugin,paymentstatus,paymenttype,paddress,paddress2,pcity,pstate,pzip,pphone,cc_number,cc_expmonth,cc_expyear,cc_transid,cc_cardtype) VALUES  ('$custname','$ccompany','$ipofpc','$amounttopay','Payeezy','ready','credit','$custaddy1','$custaddy2','$custcity','$custstate','$custzip','$custphone','$ccnumber2','$ccmonth','$ccyear','$cc_transid','$cccardtype')";
@mysqli_query($rs_connect, $rs_insert_gcc);
