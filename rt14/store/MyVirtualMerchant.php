...
start_box();
echo "<h4>".pcrtlang("Swipe Credit Card NOW!")." (MyVirtualMerchant)</h4>";

echo "<form name=c action=$securedomain/MyVirtualMerchant.php?func=add2 method=post><input autofocus class=textbox type=password name=swipedata autocomplete=off>";
...
$endsent2 = $endsent + 1;
$track2 = substr("$swipedata", 0, $endsent2);
 
$track1 = str_replace(' ', '', $track2);
                                                                                                                                              
require("header.php");

start_blue_box(pcrtlang("Add Credit Card Payment")." (MyVirtualMerchant)");
if($isdeposit != 1) {
echo "<span class=\"sizemelarger boldme\">".pcrtlang("Balance").": $money".mf("$currenttotal")."</span><br><br>";
}
echo "<table><tr><td><form action=$securedomain/MyVirtualMerchant.php?func=add3 method=post>";
...
echo "<tr><td></td><td><input type=hidden name=track1 value=\"$track1\"><input type=submit class=button value=\"".pcrtlang("Add Credit Card Payment")."\" onclick=\"this.disabled=true;this.value='".pcrtlang("Adding Credit Card Payment")."...'; this.form.submit();\"></form></td></tr></table>";
stop_blue_box();
...
(
        ...
        "ssl_merchant_id"       => "$mvm_merchant_id",
        "ssl_user_id"           => "$mvm_user_id",
        "ssl_pin"               => "$mvm_pin",
        "ssl_result_format"     => 'ascii',
        "ssl_track_data"        => "$track1",
        "ssl_card_number"       => "$ccnumber",
        "ssl_exp_date"          => "$ccexpdate",
        "ssl_card_present"      => "Y",
        "ssl_amount"            => "$amounttopay",
	"ssl_avs_address"	=> "$custaddy1",
	"ssl_city"	 	=> "$custcity",
	"ssl_state"		=> "$custstate",
	"ssl_avs_zip"		=> "$custzip",
	"ssl_cvv2cvc2"		=> "$cccid",
...
$fields = "";
foreach( $mvm_values as $key => $value ) $fields .= "$key=" . urlencode( $value ) . "&";

$ch = curl_init("$mvm_url");
...
$isapprovedarray2 = explode("\n", $resp);
foreach($isapprovedarray2 as $key => $val) {
$isapproved_indiv = explode("=", $val);
$num1 = $isapproved_indiv['0'];
$num2 = $isapproved_indiv['1'];
$isapprovedarray[$num1] = $num2;
}

if(array_key_exists('ssl_result', $isapprovedarray)) {
if($isapprovedarray[ssl_result] == 0) {
$isapproved = "yes";
$cc_transid = $isapprovedarray['ssl_txn_id'];
} else {
$isapproved = "no";
$error = $isapprovedarray['ssl_result_message'];
}
} else {
$isapproved = "no";
$error = $isapprovedarray['errorMessage'];
}
...
$rs_insert_gcc = "INSERT INTO currentpayments (pfirstname,pcompany,byuser,amount,paymentplugin,paymentstatus,paymenttype,paddress,paddress2,pcity,pstate,pzip,pphone,cc_number,cc_expmonth,cc_expyear,cc_transid) VALUES  ('$custname','$ccompany','$ipofpc','$amounttopay','MyVirtualMerchant','ready','credit','$custaddy1','$custaddy2','$custcity','$custstate','$custzip','$custphone','$ccnumber2','$ccmonth','$ccyear','$cc_transid')";
@mysqli_query($rs_connect, $rs_insert_gcc);
