...
start_box();
echo "<h4>".pcrtlang("Swipe Credit Card NOW!")." (AuthorizeNetCP)</h4><br><br>";

echo "<form name=c action=$securedomain/AuthorizeNetCP.php?func=add2 method=post><input autofocus class=textbox type=password name=swipedata autocomplete=off>";
...
start_blue_box(pcrtlang("Add Credit Card Payment")." (AuthorizeNetCP)");
if($isdeposit != 1) {
echo "<span class=sizemelarger>".pcrtlang("Balance").": $money".mf("$currenttotal")."</span><br><br>";
}
echo "<table><tr><td><form action=$securedomain/AuthorizeNetCP.php?func=add3 method=post>";
