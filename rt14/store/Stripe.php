...
start_box();
echo "<h4>".pcrtlang("Swipe Credit Card NOW")."! (Stripe)</h4>";

echo "<form name=c action=$securedomain/Stripe.php?func=add2 method=post><input autofocus class=textbox type=password name=swipedata autocomplete=off>";
...
echo "<tr><td></td><td><input type=submit class=button value=\"".pcrtlang("Add Credit Card Payment")."\"></form></td></tr></table>";
...
$ch = \Stripe\Charge::create(array(
  ...
  "source" => $token,
  "description" => pcrtlang("Charge from")." $cardname - $storeinfoarray[storename]")
);
