...
start_box();
echo "<h4>".pcrtlang("Swipe Credit Card NOW")."! (Square)</h4>";

echo "<form name=c action=$securedomain/Square.php?func=add2 method=post><input autofocus class=textbox type=password name=swipedata autocomplete=off>";
...
<div style="width:300px;margin-left:auto;margin-right:auto;margin-top:50px; background:white; padding:30px; border-radius:10px;">
<span class="sizeme16 boldme"><?php echo pcrtlang("Square Payment"); ?></span><br><br>

<?php echo pcrtlang("Please Re-enter Card Info:"); ?>
...
start_blue_box(pcrtlang("Add Credit Card Payment")." (Square)");
if($isdeposit != 1) {
echo "<span class=sizemelarger>".pcrtlang("Balance").": $money".mf("$currenttotal")."</span>";
}
echo "<form action=$securedomain/Square.php?func=add4 method=post><table>";

echo "<tr><td>".pcrtlang("Amount to Pay").": </td><td>$money <input type=text class=textboxw name=cardamount value=\"".mf("$currenttotal")."\" size=15></td></tr>";
...
echo "<tr><td></td><td><input type=submit class=button value=\"".pcrtlang("Add Credit Card Payment")."\" onclick=\"this.disabled=false;this.value='".pcrtlang("Adding Credit Card Payment")."...'; this.form.submit();\"></form></td></tr></table>";
stop_blue_box();
...
if ($isapproved == 0) {
echo "<span style=\"font-size:16px;\">".pcrtlang("Credit Card Declined")."</span><br><br>";
} else {
...
if ($isapproved == 0) {
require_once("common.php");
echo "<span style=\"font-size:16px;\">".pcrtlang("Refund Failed")."</span><br><br>";
