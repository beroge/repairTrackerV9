...
start_box();
echo "<h4>".pcrtlang("Swipe Credit Card NOW!")." (SagePayments)</h4>";

echo "<form name=c action=$securedomain/SagePayments.php?func=add2 method=post><input autofocus class=textbox type=password name=swipedata>";
...
start_blue_box(pcrtlang("Add Credit Card Payment")." (SagePayments)");
if($isdeposit != 1) {
echo "<span colormelarger>".pcrtlang("Balance").": $money".mf("$currenttotal")."</span><br><br>";
}
echo "<table><tr><td><form action=$securedomain/SagePayments.php?func=add3 method=post>";
...
$addy1ins = "UPDATE currentcustomer SET caddress = '$custaddy1' WHERE byuser = '$ipofpc' AND ccaddress = ''";
@mysqli_query($rs_connect, $addy1ins);
}
}
...
//make your query.
$data = "m_id=" . $SagePayments_mid; //your eftsecure merchant id.
$data .= "&m_key=" . $SagePayments_mkey; // your eftsecure merchant key;
$data .= "&T_amt=" . urlencode($amounttopay); // The amount to be charged. 
$data .= "&C_name=" . urlencode($custname);
$data .= "&C_address=" . urlencode($custaddy1);
$data .= "&C_state=" . urlencode($custstate);
$data .= "&C_city=" . urlencode($custcity);
$data .= "&C_cardnumber=" . urlencode($ccnumber);
$data .= "&C_exp=" . urlencode($ccexpdate);
$data .= "&C_ccv=" . urlencode($cccid);
$data .= "&C_zip=" . urlencode($custzip);
$data .= "&C_email=" . urlencode($custemail);
$data .= "&T_code=01"; // transaction type indicator now we will make the transaction. 
$ch = curl_init(); 
...
$rs_insert_gcc = "INSERT INTO currentpayments (pfirstname,pcompany,byuser,amount,paymentplugin,paymentstatus,paymenttype,paddress,paddress2,pcity,pstate,pzip,pphone,cc_number,cc_expmonth,cc_expyear,cc_transid) VALUES  ('$custname','$ccompany','$ipofpc','$amounttopay','SagePayments','ready','credit','$custaddy1','$custaddy2','$custcity','$custstate','$custzip','$custphone','$ccnumber2','$ccmonth','$ccyear','$cc_transid')";
@mysqli_query($rs_connect, $rs_insert_gcc);
...
$thereason = substr($res, 8, 32);
die (pcrtlang("Gateway Error").": $thereason");
}
...
$data = "m_id=" . $SagePayments_mid; //your eftsecure merchant id.
$data .= "&m_key=" . $SagePayments_mkey; // your eftsecure merchant key;
$data .= "&C_reference=" . urlencode($cc_transid);
$data .= "&T_code=04"; // transaction type indicator now we will make the transaction. 
$ch = curl_init(); 
curl_setopt($ch, CURLOPT_URL, $eftsecure_url); // set the URL to post to.
curl_setopt($ch, CURLOPT_POST, 1); 
curl_setopt($ch, CURLOPT_POSTFIELDS, $data); 
