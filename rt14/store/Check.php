...
start_blue_box(pcrtlang("Add Check Payment"));
if($isdeposit != 1) {
echo "<span class=\"sizemelarge boldme\">".pcrtlang("Balance").": $money".mf("$currenttotal")."</span><br><br>";
}
echo "<table><tr><td><form action=Check.php?func=add2 method=post>";
echo "".pcrtlang("Check Amount").": </td><td>$money <input type=text class=textboxw name=checkamount value=\"".mf("$currenttotal")."\" size=15></td></tr>";
echo "<tr><td>".pcrtlang("Check Number").": </td><td><input autofocus type=text class=textboxw name=checknumber size=10></td></tr>";
echo "<tr><td>".pcrtlang("Drivers License Number").": </td><td><input type=text class=textboxw name=driverslc size=30 onFocus=\"this.form.submitbutton.disabled=false;this.form.submitbutton.value='".pcrtlang("Add Check Payment")."';\"></td></tr>";
echo "<tr><td>".pcrtlang("Phone").": </td><td><input type=text class=textboxw name=cphone value=\"$cphone\" size=20></td></tr>";

echo "<tr><td colspan=2>&nbsp;</td></tr>";
echo "<tr><td colspan=2>&nbsp;</td></tr>";

echo "<tr><td>".pcrtlang("Name on Check").": </td>";
...
echo "<input type=text class=textboxw name=czip value=\"$czip\" size=10><br>";
...
echo "<tr><td></td><td><input type=submit class=button id=submitbutton value=\"".pcrtlang("Add Check Payment")."\" onclick=\"this.disabled=true;this.value='".pcrtlang("Adding Check Payment")."...'; this.form.submit();\"></form></td></tr></table>";
stop_blue_box();
...
$amounttopay = $_REQUEST['checkamount'];
$checknumber = pv($_REQUEST['checknumber']);
$driverslc = pv($_REQUEST['driverslc']);

if (("$amounttopay" > "$pcrt_driverslc_minimum") && ($driverslc == "" )){
die("Please go back and enter a drivers license number");
}
...
$rs_insert_check = "INSERT INTO deposits (pfirstname,pcompany,byuser,amount,paymentplugin,paymentstatus,paymenttype,paddress,paddress2,pcity,pstate,pzip,pphone,pemail,chk_dl,chk_number,woid,invoiceid,dstatus,depdate,storeid,registerid) VALUES  ('$custname','$ccompany','$ipofpc','$amounttopay','Check','ready','check','$custaddy1','$custaddy2','$custcity','$custstate','$custzip','$custphone','$custemail','$driverslc','$checknumber','$woid','$invoiceid','open','$currentdatetime','$defaultuserstore','$registerid')";
...
$rs_insert_check = "INSERT INTO currentpayments (pfirstname,pcompany,byuser,amount,paymentplugin,paymentstatus,paymenttype,paddress,paddress2,pcity,pstate,pzip,pphone,pemail,chk_dl,chk_number) VALUES  ('$custname','$ccompany','$ipofpc','$amounttopay','Check','ready','check','$custaddy1','$custaddy2','$custcity','$custstate','$custzip','$custphone','$custemail','$driverslc','$checknumber')";
@mysqli_query($rs_connect, $rs_insert_check);
...
if ($isdeposit != 1) {
$rs_void_check = "DELETE FROM currentpayments WHERE paymentid = '$payid'";
@mysqli_query($rs_connect, $rs_void_check);
header("Location: cart.php");
} else {
$rs_void_check = "DELETE FROM deposits WHERE depositid = '$depositid'";
@mysqli_query($rs_connect, $rs_void_check);
