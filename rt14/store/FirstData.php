...
start_box();
echo "<h4>".pcrtlang("Swipe Credit Card NOW!")." (FirstData)</h4>";

echo "<form name=c action=$securedomain/FirstData.php?func=add2 method=post><input autofocus class=textbox type=password name=swipedata>";
...
start_blue_box(pcrtlang("Add Credit Card Payment")." (FirstData)");
if($isdeposit != 1) {
echo "<span class=\"sizemelarge boldme\">".pcrtlang("Balance").": $money".mf("$currenttotal")."</span><br><br>";
}
echo "<table><tr><td><form action=$securedomain/FirstData.php?func=add3 method=post>";
...
echo "<tr><td>".pcrtlang("Expiration Year").": </td><td><input type=text class=textboxw name=ccyear size=4 value=\"$ccexpyear\"> <span class=\"sizemesmaller italme\">".pcrtlang("2 digits")."</span></td></tr>";
...
$data = array(
"gateway_id" 		=> "$firstdataglobale4_gatewayid",
"password" 		=> "$firstdataglobale4_password",
"transaction_type" 	=> "00",
"amount" 		=> "$amounttopay",
"cardholder_name" 	=> "$cfirstname $clastname",
"cc_number" 		=> "$ccnumber",
"cc_expiry" 		=> "$ccexpdate",
"cc_verification_str1"  => "$custaddy1|$custzip|$custcity|$custstate|USA",
"zip_code"              => "$custzip",
"client_email"          => "$custemail",
"cc_verification_str2"  => "$custaddy1",
);

$data_string= json_encode($data);

// Initializing curl
$ch = curl_init( $firstdataglobale4_url );
...
if ($data_string) {
   if ($data_string->Transaction_Approved == 'true') {
$isapproved = "yes";
   } else {
$firstdataerror = "$data_string->bank_message";
$isapproved = "no";
   }
} else {
$firstdataerror = ($result);
$isapproved = "no";
}
...
$rs_insert_gcc = "INSERT INTO deposits (pfirstname,pcompany,byuser,amount,paymentplugin,paymentstatus,paymenttype,paddress,paddress2,pcity,pstate,pzip,pphone,cc_number,cc_expmonth,cc_expyear,cc_transid,cc_cardtype,invoiceid,woid,dstatus,depdate,storeid,registerid) VALUES  ('$custname','$ccompany','$ipofpc','$amounttopay','FirstData','ready','credit','$custaddy1','$custaddy2','$custcity','$custstate','$custzip','$custphone','$ccnumber2','$ccmonth','$ccyear','$cc_transid','$cccardtype','$woid','$invoiceid','open','$currentdatetime','$defaultuserstore','$registerid')";
...
$rs_insert_gcc = "INSERT INTO currentpayments (pfirstname,pcompany,byuser,amount,paymentplugin,paymentstatus,paymenttype,paddress,paddress2,pcity,pstate,pzip,pphone,cc_number,cc_expmonth,cc_expyear,cc_transid,cc_cardtype) VALUES  ('$custname','$ccompany','$ipofpc','$amounttopay','FirstData','ready','credit','$custaddy1','$custaddy2','$custcity','$custstate','$custzip','$custphone','$ccnumber2','$ccmonth','$ccyear','$cc_transid','$cccardtype')";
@mysqli_query($rs_connect, $rs_insert_gcc);
