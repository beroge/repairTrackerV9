...
start_box();
echo "<h4>".pcrtlang("Swipe Credit Card NOW!")." (AuthorizeNet)</h4><br><br>";

echo "<form name=c action=$securedomain/AuthorizeNet.php?func=add2 method=post><input autofocus class=textbox type=password name=swipedata autocomplete=off>";
...
start_blue_box(pcrtlang("Add Credit Card Payment")." (AuthorizeNet)");
if($isdeposit != 1) {
echo "<h4>".pcrtlang("Balance").": $money".mf("$currenttotal")."</h4><br><br>";
}
echo "<table><tr><td><form action=$securedomain/AuthorizeNet.php?func=add3 method=post>";
