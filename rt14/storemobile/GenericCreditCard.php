...
dheader(pcrtlang("Add Credit Card Payment")." (GenericCreditCard)");
if ($isdeposit != 1) {
echo pcrtlang("Balance").": $money".mf("$currenttotal")."<br><br>";
}
echo "<form action=GenericCreditCard.php?func=add2 method=post data-ajax=\"false\">";
echo pcrtlang("Amount to Pay").": <input type=text name=cardamount value=\"".mf("$currenttotal")."\">";
echo pcrtlang("Last 4 CC Digits").": <input type=text name=ccnumber>";
echo pcrtlang("Expiration Month (2 digits)").": <input type=text name=ccmonth>";
echo pcrtlang("Expiration Year (4 digits)").": <input type=text name=ccyear>";
echo pcrtlang("Confirmation Number").": <input type=text name=ccconf>";

echo "<br>";

echo pcrtlang("Name on Card").": ";
