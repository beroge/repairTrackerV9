...
dheader(pcrtlang("AuthorizeNet"));
echo pcrtlang("Swipe Credit Card NOW!")." (AuthorizeNet)<br><br>";

echo "<form name=c action=$securedomain"."mobile/AuthorizeNet.php?func=add2 method=post data-ajax=\"false\"><input autofocus type=password name=swipedata>";
...
dheader("AuthorizeNet");
if($isdeposit != 1) {
echo pcrtlang("Balance").": $money".mf("$currenttotal")."<br><br>";
}
echo "<form action=$securedomain"."mobile/AuthorizeNet.php?func=add3 method=post data-ajax=\"false\">";
...
echo pcrtlang("Expiration Month (2 digits)").": <input type=text name=ccmonth size=4 value=\"$ccexpmonth\">";
echo pcrtlang("Expiration Year").": <input type=text name=ccyear value=\"$ccexpyear\">";
...
echo "<input type=submit value=\"".pcrtlang("Add Credit Card Payment")."\" onclick=\"this.disabled=true;this.value='".pcrtlang("Adding Credit Card Payment")."...'; this.form.submit();\"></form>";
dfooter();
...
(
        ...
        "x_type"                => "AUTH_CAPTURE",
        "x_method"              => "CC",
        "x_tran_key"            => $AuthorizeNetTranKey,
...
$rs_insert_gcc = "INSERT INTO currentpayments (pfirstname,pcompany,byuser,amount,paymentplugin,paymentstatus,paymenttype,paddress,paddress2,pcity,pstate,pzip,pphone,cc_number,cc_expmonth,cc_expyear,cc_transid,cc_cardtype) VALUES  ('$custname','$ccompany','$ipofpc','$amounttopay','AuthorizeNet','ready','credit','$custaddy1','$custaddy2','$custcity','$custstate','$custzip','$custphone','$ccnumber2','$ccmonth','$ccyear','$cc_transid','$cccardtype')";
@mysqli_query($rs_connect, $rs_insert_gcc);
