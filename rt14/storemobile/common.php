...
function start_gray_box($gray_title) {
        ...
        echo "<div class=colortitletopround style=\"$boxstyle[headerstyle]\"><font class=textwhite16b>$gray_title</font></div><div class=whitebottom style=\"$boxstyle[bodystyle]\">";
}
...
function start_color_box($statusid,$title) {
        $boxstyle = getboxstyle($statusid);
        echo "<div class=\"colortitletopround\" style=\"$boxstyle[headerstyle]\">\n";
        echo "<font class=textwhite16b>$title</font></div>\n";
        echo "<div class=whitebottom style=\"$boxstyle[bodystyle]\">\n";
}
...
function start_color_boxnobottomround($statusid,$title) {
        ...
        echo "<div class=\"colortitletopround\" style=\"$boxstyle[headerstyle]\">\n";
        echo "$title</div>\n";
        echo "<div class=whitemiddle style=\"$boxstyle[bodystyle]\">\n";
}

function start_color_boxnoround($statusid,$title) {
        $boxstyle = getboxstyle($statusid);
        echo "<div class=\"colortitle\" style=\"$boxstyle[headerstyle]\">\n";
        echo "<font class=textwhite16b>$title</font></div>\n";
        echo "<div class=whitebottom style=\"$boxstyle[bodystyle]\">\n";
}
...
echo "<img src=images/icon/error.png align=right>";
}

function info_image() {
echo "<img src=images/icon/dialog_info.png align=right>";
}       
...
echo "<h3>".pcrtlang("Stock Categories")."</h3>";
...
echo "<h3>".pcrtlang("Ready to Sell")."</h3>";
...
$rs_findassets = "SELECT * FROM pc_wo,pc_owner WHERE pc_wo.pcstatus = '7' AND pc_owner.mainassettypeid = '$mainassettypeid'
AND pc_wo.pcid = pc_owner.pcid AND pc_wo.storeid = '$defaultuserstore'";
...
echo "<button type=button onClick=\"parent.location='cart.php?func=show_savecarts&mainassettypeid=$mainassettypeid'\"
 class=\"ui-btn ui-corner-all ui-shadow ui-mini\">$mainassetname ($numberofassets)</button>";

}
}

echo "<button type=button onClick=\"parent.location='cart.php?func=show_savecarts'\" class=\"ui-btn ui-corner-all ui-shadow ui-mini\">".pcrtlang("All")." ($totalassets)</button>";
...
echo "<button type=button onClick=\"parent.location='stock.php?func=show_stock&category=$rs_subcatid'\" class=\"ui-btn ui-corner-all ui-shadow ui-mini\">$rs_subcatname ($rs_subcat_items)</button>";
}
...
function get_paged_nav($num_results, $num_per_page=40, $show=false)
{
    ...
    for($i=1; $i<=$total_pages; $i++)
    {
      ...
      //     OR $i == floor($total_pages/2)+5
                ...
                {
                    $output .= "<button type=button onClick=\"parent.location='$new_url'\"  class=\"ui-btn ui-corner-all ui-shadow ui-btn-inline\">$i</button> ";
                }
                else
                    $output .= '. ';
        }
...
if($totalready > 0) {
$pcrtnotify .= "<button type=button  data-theme=\"b\"  data-mini=\"true\" onClick=\"parent.location='../storemobile/rinvoice.php?func=runinvoices'\"><i class=\"fa fa-check-circle fa-lg\"></i> ".pcrtlang("Recurring Invoices")."</button>";
}

}

if (perm_check("17")) {
$findsrsql = "SELECT * FROM servicereminders WHERE srdate < NOW() AND srsent = '0'";
$findsrq = @mysqli_query($rs_connect, $findsrsql);
$totalsr = mysqli_num_rows($findsrq);
if($totalsr > 0) {
$pcrtnotify .= "<button type=button  data-theme=\"b\"  data-mini=\"true\" onClick=\"parent.location='../repairmobile/servicereminder.php?func=runsr'\"><i class=\"fa fa-check-circle fa-lg\"></i> ".pcrtlang("Service Reminders")."</button>";
}
}
...
if($totalsreq > 0) {
$pcrtnotify .= "<button type=button  data-theme=\"b\"  data-mini=\"true\" onClick=\"parent.location='../repairmobile/servicerequests.php?func=runsreq'\"><i class=\"fa fa-check-circle fa-lg\"></i> ".pcrtlang("Service Requests")."</button>";
}
...
if($totalsrecount2 > 0) {
$pcrtnotify .= "<button type=button  data-theme=\"b\"  data-mini=\"true\" onClick=\"parent.location='categories.php?func=stockrecount'\"><i class=\"fa fa-check-circle fa-lg\"></i> ".pcrtlang("Inventory Recount Requested.")."</button>";
}
}
...
echo "<button onClick=\"window.history.back()\" class=bigbutton><img src=../store/images/left.png style=\"vertical-align:middle;margin-bottom: .25em;\"> ".pcrtlang("Return")."</button>";
